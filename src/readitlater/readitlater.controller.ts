import { Controller, UseGuards, Get, Body,Post, ClassSerializerInterceptor,UseInterceptors } from '@nestjs/common';
...
import { RecommendationDto, CreateRecommendationDto } from './dto/recommendation.dto';
...
export class ReaditlaterController {
    ...
    async sendRecommendation(@Body() recommendation : CreateRecommendationDto, @User() user : UserDto) {
      const result = await this.readItLaterService.sendRecommendation(recommendation, user.id);
