...
import { CreateRecommendationDto, ReferralDto, ReferenceDto } from './dto/recommendation.dto';
...
export class ReaditlaterService {
    ...
    async saveArticle(data : CreateArticleDto, id : number ) {
        ...
        if ( !user ) throw new HttpException({ status: HttpStatus.NOT_FOUND, message: 'User does not exists!'}, HttpStatus.NOT_FOUND);
    ...
    async sendRecommendation(data : CreateRecommendationDto, id : number ) {
        ...
        if ( !referral ) throw new HttpException({ status: HttpStatus.NOT_FOUND, message: 'User Referral does not exists!'}, HttpStatus.NOT_FOUND);
    ...
    async getRecommendations(id : number) {
        let recommendations = await this.repoRecommend.find({ select : ['id','url','entryDate',"referral"], where : { user : id }, relations : ['referral']});
